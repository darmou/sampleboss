...
from djangobosssearch.pagerank import GoogleRank
from djangobosssearch.alexarank import AlexaRank
...
class BossResultGenerator(object):
    ...
    def next(self):
        ...
        if self.i < len(self._raw_resultset):
            ...
            self.i += 1
            return BossResult(result,self.i)
        raise StopIteration
...
class BossResult(base.Result):
    ...
    def __init__(self, raw_result, index):
        pr = GoogleRank()
        ar = AlexaRank()
        ...
        #f.close()
        self._googlerank = pr.get_rank(self._url)
        self._alexarank = ar.get_poplarity_rank(self._url)
